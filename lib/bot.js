...
function bot(torrent, bot) {
  ...
  bot.onText(downloadRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      bot.sendMessage(from, "Link is not a Magnetic link! You can try with our @Files_Uploader_Robot");
    } else {
  ...
  bot.onText(statusRegex, (msg, match) => {
    ...
    if (link.indexOf("magnet:") !== 0) {
      bot.sendMessage(from, "Link is not a magnet link! You can try with our @Files_Uploader_Robot");
    } else if (!torr) {
  ...
  bot.onText(removeRegex, (msg, match) => {
    ...
    try {
      torrent.remove(link);
      bot.sendMessage(from, "Removed to Que");
    } catch (e) {
